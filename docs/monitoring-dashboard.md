...
Status: **implemented** on the Node SQLite server (`apps/server`). Not on the Cloudflare Worker.

| Route | Auth |
| --- | --- |
| `GET /admin` and `GET /admin/monitor` | `CODEFRIENDS_ADMIN_TOKEN` cookie after `POST /admin/session`, or Bearer / `X-Admin-Token`. No cookie → login form (only when the env token is set). |
| `GET /metrics` | Bearer, `X-Admin-Token`, or cookie. JSON snapshot. |
| `GET /health` | Public, unchanged. |

Token unset or wrong → **404** empty body (not the popout). Cookie: `HttpOnly`, `SameSite=Strict`, `Path=/admin`, `Secure` on HTTPS.

This is the implementation of the spec that started in PR #17. Phase 4 (outbound webhook, host RAM/disk) and phase 5 (out-of-process poller) are still later.

Audience: whoever runs `apps/server` on the Windows self-host that serves [https://codefriends.1ststep.ai](https://codefriends.1ststep.ai).
...
- [x] Spec-only constraint lifted; this file lists the real routes (`/admin`, `/admin/monitor`, `/metrics`).
- [x] `GET /health` response is unchanged: `ok`, `name`, `onlineCount`, `store`, `dmHistoryLimit` only.
- [x] Worker bundle has no `/metrics` and no `/admin/monitor` (Node `apps/server` only).
- [x] With `CODEFRIENDS_ADMIN_TOKEN` unset, `GET /metrics` and `GET /admin` are 404 and are not the popout `index.html`.
- [x] With the token set, wrong token is 404; right token returns JSON including `uptimeSec`, `memory`, histogram, `sqlite.busy`, `sqlite.writeLatencyMs`, `sqlite.connections` (1 for better-sqlite3), `lastError`, `onlineCount`, tunnel block, plus `auth.gemini` / `auth.google`.
- [x] A deliberately slow write in a test (>50 ms and >200 ms) moves the gauge threshold. The test does not use a connection pool.
- [x] `GET /admin` shows status strip, last-hour sparkline, write gauge, online count series, last 20 errors, tunnel status. No popout navigation entry points at it.
- [x] Samples land in `<dir of CODEFRIENDS_DB>/monitor/` (or `CODEFRIENDS_MONITOR_DIR`) as daily JSONL, not in `codefriends.sqlite`.
- [x] Files older than 7 days are removed by the sampler. Tests inject clock timestamps rather than waiting.
- [x] `alerts.jsonl` gains a `sqlite.write` line when a write exceeds 200 ms, and does not gain a duplicate of the same key inside 15 minutes.
- [x] Unset webhook URL causes zero outbound alert requests (webhooks not implemented; phase 4).
- [x] `CODEFRIENDS_AGENT_MEMORY_PATH` unset → no error. Set to a missing path → no error. Set to an existing file → one line appended.
- [x] Tunnel check uses the local process list (and `listeningLocal` from the HTTP server). It does not read cloudflared credential JSON, and does not change `scripts/windows/*.cmd`.
- [x] Error entries contain no DM body, session token, OAuth secret, or SQL bound parameters.
- [x] `npm run smoke` covers the admin 404/200 split and prune. Smoke's temp DB does not require a webhook.
- [x] `.env.example` documents the new variables as comments, with empty secrets.
- [x] Self-host doc and README point at the page and the token.
